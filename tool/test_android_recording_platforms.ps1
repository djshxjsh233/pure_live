...
$expected = @('bilibili','douyu','huya','douyin','kuaishou')
...
foreach ($Platform in $expected) {
 ...
 if (-not $supported) { throw "Incorrect remote chat capability for $Platform" }
...
if ($table['huya'] -ne $translations.site_huya) { throw 'Huya navigation label differs from the actual translation' }
Write-Host 'PASS Huya localized label'
...
foreach ($foreignPlatform in @('douyu', 'huya')) {
