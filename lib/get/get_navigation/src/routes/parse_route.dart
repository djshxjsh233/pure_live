...
class RouteDecoder {
  ...
  factory RouteDecoder.fromRoute(String location) {
    ...
    decoder.route = decoder.route?.copyWith(completer: null, arguments: args, parameters: args.params);
    return decoder;
  }
...
class ParseRouteTree {
  ...
  RouteDecoder matchRoute(String name, {PageSettings? arguments}) {
    ...
    if (treeBranch.isNotEmpty) {
      ...
      final mappedTreeBranch = treeBranch
          .map(
            (e) => e.value.copyWith(
              parameters: {if (e.value.parameters != null) ...e.value.parameters!, ...params},
              name: e.key,
            ),
          )
          .toList();
  ...
  List<GetPage> _flattenPage(GetPage route) {
    ...
    for (var page in route.children) {
      ...
      result.add(_addChild(page, parentPath, parentMiddlewares, parentBindings, parentBinds));
      ...
      for (var child in children) {
        result.add(
          _addChild(
            child,
            parentPath,
            [...parentMiddlewares, if (child.middlewares.isNotEmpty) ...child.middlewares],
            [
              ...parentBindings,
  ...
  GetPage? _findRoute(String name) {
    final value = routes.firstWhereOrNull((route) => route.path.regex.hasMatch(name));
