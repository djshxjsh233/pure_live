...
class GetListenable<T> extends ListNotifierSingle implements RxInterface<T> {
  ...
  StreamSubscription<T> listen(
    ...
    bool? cancelOnError,
  }) => stream.listen(onData, onError: onError, onDone: onDone, cancelOnError: cancelOnError ?? false);
    ...
    return Observer(
      builder: (context) {
        if (status.isLoading) {
          ...
          return onCustom?.call(context) ?? const SizedBox.shrink(); // Also can be widget(null); but is risky
        }
        return widget(value);
      },
    );
