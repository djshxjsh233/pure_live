...
class BarrageEngine extends FlameGame with TapCallbacks {
  ...
  double _calculateAllowedHeight(double rawHeight) {
    ...
    final double allowed = (rawHeight * _config.area) - finalTop - finalBottom;
    // 加固：避免全屏/横屏等小尺寸或底部 inset 过大时算出 <=0 的高度，
    // 否则轨道恒为空导致弹幕被整体丢弃。
    if (allowed <= 0) return _config.trackHeight.isFinite ? _config.trackHeight : 50.0;
    return allowed;
  }
